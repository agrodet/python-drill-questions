...
mds = MDS(n_components="""①""", random_state=0)
...
for name, transformer in [('pca', pca), ('mds', mds), ('kbest', kbest)]:
    transformed = transformer."""②"""(dataset.data, dataset.target)
    accuracy = cross_val_score(model, """③""", dataset.target, cv=kfold, scoring="accuracy")
